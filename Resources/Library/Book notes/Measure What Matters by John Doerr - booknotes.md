---
title: Measure What Matters (John Doerr)
author: John Doerr
year: 2018
tags:
  books
  leadership
  productivity
  goal_setting
  OKRs
  business_strategy
  personal_development
---

# Summary/Plot  
"Measure What Matters" by John Doerr introduces **Objectives and Key Results (OKRs)**, a goal-setting framework adopted by companies like Google, Intel, and the Gates Foundation to align organizational priorities and drive measurable success. The book combines historical case studies, interviews, and actionable frameworks to demonstrate how OKRs foster focus, accountability, and collaboration. Doerr, a venture capitalist, traces OKRs’ origins to Andy Grove’s work at Intel and their modern application in scaling tech giants and social impact initiatives. 

Key themes include:  
- **Focus on outcomes, not activities**: Prioritize what truly matters.  
- **Transparency**: Share goals across all levels of an organization.  
- **Stretch goals**: Ambitious targets that push beyond comfort zones.  
- **Continuous feedback**: Regular check-ins to track progress.  

# Key Takeaways & Learnings  
## 1. **OKR Framework**  
   - **Objectives**: Qualitative, aspirational goals (e.g., "Launch a groundbreaking product").  
   - **Key Results**: Quantitative, time-bound metrics (e.g., "Achieve 100,000 pre-orders by Q3").  
   - Example: Google’s early OKR to "Organize the world’s information" with KR: "Index 1 billion web pages by 2001". 

## 2. **Superpowers of OKRs**  
   - **Focus & Commitment**: Eliminate distractions by narrowing priorities. 
   - **Teamwork**: Align individual efforts with organizational goals.  
   - **Accountability**: Clear metrics reduce ambiguity.  
   - **Stretch Goals**: Encourage innovation beyond incremental improvements. 

## 3. **Common Pitfalls to Avoid**  
   - Vague Key Results (e.g., "Improve customer satisfaction" vs. "Increase NPS to 70 by end of year").  
   - Overloading with too many OKRs (limit to 3–5 objectives per team).  
   - Ignoring cultural alignment (OKRs require trust and transparency). 

## 4. **Historical Context**  
   - Andy Grove’s influence: Grove’s Management by Objectives (MBO) system at Intel laid the foundation for modern OKRs.  
   - Bono’s ONE Campaign: Used OKRs to coordinate global efforts against extreme poverty [[2]][[5]].  

# Quotes & Explanations  
> "When we choose to measure something, we imply it matters."  
> – John Doerr  
>  
> **Meaning**: Metrics shape behavior. Choosing the right KRs ensures energy is directed toward meaningful outcomes [[10]].  

> "OKRs are not a straightjacket; they’re a compass."  
> – John Doerr  
>  
> **Meaning**: Flexibility is key. Adjust OKRs as circumstances change, but never lose sight of the objective [[7]].  

# Practical Applications  
## Daily Habits  
- **Morning Check-In**: Spend 5 minutes reviewing daily OKRs to align tasks with priorities.  
- **Progress Journaling**: Track small wins tied to KRs (e.g., "Completed 80% of client onboarding surveys").  

## Weekly/Monthly Activities  
- **OKR Reviews**: Dedicate 30 minutes weekly to assess KR progress.  
- **Team Alignment Sessions**: Use collaborative tools like **Miro** or **Trello** to visualize OKRs and dependencies.  

## Assessments  
- **OKR Grading**: Score KRs on a 0–1 scale (0.7 = "good but incomplete"). Focus on learning, not punishment [[4]].  
- **360-Degree Feedback**: Gather input from peers, managers, and subordinates to refine objectives.  

# Related Notes & Connections  
- **Peter Drucker’s MBO**: OKRs evolved from Drucker’s Management by Objectives but emphasize greater transparency and stretch goals.  
- **SMART Goals**: OKRs overlap with SMART criteria but prioritize ambition over realism for KRs.  
- **Agile Methodology**: OKRs complement Agile by aligning long-term goals with iterative execution.  

# Similar Books & Articles  
| Title | Author | Connection |  
|-------|--------|------------|  
| *The 4 Disciplines of Execution* | Chris McChesney | Focuses on executing top priorities in a distracted world. |  
| *Radical Focus* by Christina Wodtke | Christina Wodtke | Explores OKRs through fictionalized case studies. |  
| *HBR’s Guide to OKRs and KPIs* | Harvard Business Review | Practical advice on balancing OKRs with traditional KPIs. |  

# Online Resources  
## Videos  
- [John Doerr’s TED Talk: "How to Set Goals to Achieve Your Most Ambitious Dreams"](https://www.youtube.com/watch?v=QqcVbeLsNhc )  
- [OKR Explained in 5 Minutes](https://www.youtube.com/watch?v=ZV5QgLO6WjY )  

## Freebies & Tools  
- **OKR Templates**: Download free templates from [Atlassian](https://www.atlassian.com/agile/okrs/templates ) or [Weekdone](https://weekdone.com/okr-template/ ).  
- **Printables**: OKR checklists and progress trackers from [OKR Institute](https://okrinstitute.org/ ).  

## Communities  
- **OKR Slack Groups**: Join forums like [OKR Community](https://okrcommunity.com/ ) for peer discussions.  
- **LinkedIn Groups**: Search for "OKR Practitioners" for case studies and networking.  

# Graphics & Visuals  
![](https://upload.wikimedia.org/wikipedia/commons/thumb/7/7d/OKR_cycle.png/800px-OKR_cycle.png )  
*OKR Cycle: Set → Align → Track → Grade*  

# Obsidian Integration  
- **Linked Notes**:  
  - [[Andy Grove]]  
  - [[Goal Setting Theory]]  
  - [[Agile Project Management]]  
- **Callouts**:  
  > [!tip]  
  > Start with pilot teams before rolling out OKRs company-wide.  
  >  
  > [!summary]  
  > OKRs thrive in cultures that value transparency and psychological safety.  
